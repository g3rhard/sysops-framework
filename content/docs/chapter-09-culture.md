...
| ------------------------------------------------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------- | ------------------------------------------------------- | ---------------------------------------------------------------------------------- | ---------------------------------------------------------------------------------------------------------------------- |
| **Principal Engineer** (also: Staff Engineer, Tech Lead)                                   | Technical vision and architecture decisions that affect reliability, cost, and scalability                                        | Monthly Strategy — sets technical direction             | Capacity & Performance (4), Financial Management (11)                              | "Which cloud region should we expand to?" "Monolith or microservices?" "When do we migrate off this vendor?"           |
| **Architect** (also: Solutions Architect, Systems Architect)                               | System design, integration patterns, technology selection within the vision                                                       | Monthly Strategy — reviews and proposes                 | Change & Configuration (3), Release Management (8), Asset Management (9)           | "Which monitoring stack fits our topology?" "How do we design for multi-region failover?" "What's the migration path?" |
| **Engineering Manager** (also: Team Lead, Team Leader, Delivery Lead, Process Owner)       | People management, prioritization, stakeholder communication, removing blockers                                                   | Weekly Improvement — runs the retro and priority review | Team & Skill Development (6), Knowledge & Documentation (5), Vendor & Contract (7) | "Who is on-call next week?" "What improvement do we fund this quarter?" "Can the team absorb this new service?"        |
| **Platform Manager** (also: Platform Owner, Infrastructure Lead, SRE Manager)              | Infrastructure platform health, cost control, vendor relationships, SLO negotiation                                               | Daily Operations — owns the dashboard and alert triage  | Service Level Management (1), Financial Management (11), Vendor & Contract (7)     | "Do we accept this SLO?" "Is the platform bill within budget?" "Which alert gets a runbook?"                           |
| **Service Owner** (also: Service Manager, Product Owner for platform)                      | Day-to-day health of a specific service (e.g., Azure, Kubernetes, a database cluster); first responder for incidents affecting it | Daily Operations — incident response participant        | Incident & Problem (2), Backup & Recovery (12), Capacity & Performance (4)         | "Is this alert worth paging over?" "Do I roll back or fix forward?" "When does this service need more capacity?"       |
| **On-Call Engineer** (also: Duty Engineer, Incident Responder)                             | Rotating responsibility for incident response during assigned shifts (where 24/7 rotation applies)                                | Daily Operations — shift execution                      | Incident & Problem (2), Knowledge & Documentation (5)                              | "Is this a SEV1 or SEV2?" "Do we need to wake up the Service Owner?" "Is the runbook correct?"                         |
| **Incident Commander** (also: Incident Manager, Bridge Lead)                               | Coordination during major incidents — does NOT debug, keeps the response organized                                                | Daily Operations — activated per incident               | Incident & Problem (2)                                                             | "Who is on the bridge?" "Do we escalate to the director?" "What do we tell customers?"                                 |
| **SRE** (also: Reliability Engineer, Ops Engineer, Operations Engineer, Platform Engineer) | Automation, monitoring, reducing toil, improving system reliability across the platform                                           | Weekly Improvement — implements automation projects     | Release Management (8), Service Request (10), Change & Configuration (3)           | "Which manual step do I automate next?" "Is this change safe to standardize?"                                          |

> **Roles are not people.** One person may wear multiple hats (especially in small teams). The Principal Engineer may also serve as Engineering Manager. The point of the table is to make sure _every hat has a head_ — not to prescribe headcount. If a responsibility has no owner, it will be neglected; if two people think they own it, you have a future conflict. Use this table to check: for each practice in [Chapter 6](chapter-06-practices.md), can you name who drives it?
...
| **Weekly** (improvement retro) | Engineering Manager                                        | Principal Engineer, Platform Manager, rotating Service Owner |
| **Monthly** (strategy)         | Principal Engineer or Engineering Manager                  | Architect, Platform Manager, invited Service Owners          |

## Minimum Team Operating Kit

Culture becomes real through the artifacts people update and the habits they repeat. Start with one ticket shape, one service page, and a few rituals that produce decisions. Do not launch a documentation program.

### A Typical Improvement Ticket

Use the team's existing ticket system. The [work-intake template](../../templates/work-intake-triage.md) defines the full intake fields; an improvement ticket can stay this small:

```text
OPS-142  Reduce noisy disk-capacity paging

Type:        Improvement
Service:     Billing API
Cycle:       Weekly
Owner:       Sam
Why now:     11 pages in 14 days; 9 required no action
Change:      Alert on projected exhaustion within 24h, not fixed 80% usage
Done when:   Rule deployed, runbook linked, no false page for 7 days
Evidence:    Dashboard link + alert history
Related:     INC-318, INC-327
```

The useful part is not the fields; it is the chain from pain to owner to observable completion. If a ticket cannot say what changes when it is done, it is probably a topic, not ready work.

### A Minimum Service Page

Keep one page per service in the wiki, repository, or Confluence. Link to maintained sources instead of copying their contents.

```text
# Billing API

Owner: Platform Team / Sam
Tier: Tier 1 - Mission Critical
Purpose: Creates invoices and exposes billing status to customers
SLO: 99.95% monthly availability; p95 latency < 300 ms

Dashboard: <link>
Alerts and on-call route: <link>
Runbooks: <links>
Dependencies: PostgreSQL, identity service, payment provider
Recent and planned changes: <link to change queue>
Recovery: RTO 1 hour; RPO 15 minutes
Known risks: <link to risk register>
Last reviewed: <YYYY-MM-DD>
```

The [service inventory template](../../templates/service-inventory.md) is the index across all services; the service page holds the context responders need. Review it after a significant incident or ownership change, not because the calendar says every page needs rewriting.

### The Smallest Useful Ritual Set

| Ritual                        | Cadence                    | Input                                             | Required output                                               | Skip or stop when                                                         |
| ----------------------------- | -------------------------- | ------------------------------------------------- | ------------------------------------------------------------- | ------------------------------------------------------------------------- |
| **Daily operations review**   | 15 minutes or async        | Service state, new interrupts, blocked work       | Reclassified work, named owners, explicit escalations         | Nothing changed and the async update already answers those questions      |
| **Shift handoff**             | Every responder change     | Active incidents, recent changes, noisy alerts    | Acknowledged [handoff](../../templates/shift-handoff.md)      | No shift or responsibility change occurred                                |
| **Weekly improvement review** | 30–45 minutes              | Recurring pain, PIR actions, improvement evidence | One protected improvement and closure decisions               | No capacity is protected; fix that conflict instead of pretending to plan |
| **Monthly strategy review**   | 60 minutes                 | Risk, capacity, cost, service trends              | Funded priorities, deferred items, owners, trade-offs         | Nobody present can make or approve a trade-off                            |
| **Post-incident review**      | After qualifying incidents | Timeline, impact, response evidence               | Owned actions with dates, or an explicit “no action” decision | The severity rule does not require one and no learning would result       |

Meetings are optional; outputs are not. A distributed team may run daily review and handoff asynchronously. If a ritual produces no decision, owner, evidence, or changed behavior for a month, shorten it, combine it, or remove it.
...
### Scenario: Change Readiness Assessment
...
The most useful career ladder is one that tells an engineer exactly what "the next level looks like" in terms of observable behavior, not years served. Below is a concrete ladder for an operations track. Adapt the level names and scope to match your organization's existing structure.
...
| L4    | Staff Ops Engineer     | Cross-team, architectural        | Defines operational standards adopted by multiple teams; designs DR plans; influences platform architecture; publishes post-incident analyses that change team practice       | Standards adopted by 2+ teams; DR plan tested to target RTO; recognized as subject-matter expert in at least one domain        |
| L5    | Principal Ops Engineer | Organization-wide, strategic     | Sets multi-year operational strategy; designs organizational reliability practices; represents operations in executive decision-making; published external thought leadership | Organization-wide reliability improvements attributable to their work; external recognition (conference talk, published paper) |
...
| **Squad rotation**           | Entire squad rotates (primary + secondary within the squad); other squads on escalation           | Multi-team orgs with specialized squads        |
...
> **Template:** use [`templates/shift-handoff.md`](../../templates/shift-handoff.md) as the copy-ready version of this checklist - fill it in at the end of every shift, not from memory the next morning.
...
- **Alert severity must match business impact**: use the SEV1-SEV4 model consistently (see [Chapter 6, Incident Severity Classification](chapter-06-practices.md)); SEV1 wakes someone up, SEV4 goes to a ticket queue
...
| MTTA (Mean Time to Acknowledge) | Time from page to acknowledge                  | SEV1 ≤ 5 min; SEV2 ≤ 15 min          |
| Responder burnout index         | % of responders who escalated or missed a page | 0%; any miss triggers review         |

#### Escalation Path Design

> **Diagram:** SEV1 escalation from primary responder through secondary, team leadership, engineering leadership, and customer communication.

```mermaid
graph LR
  A[SEV1 Alert fires] -->|0 min| B[Page Primary on-call]
...
Document this path in the incident management runbook ([Chapter 6, Practice 2](chapter-06-practices.md#2--incident-and-problem-management)) and validate it during DR simulations.
...
- [`templates/shift-handoff.md`](../../templates/shift-handoff.md) - the handoff document exchanged at every shift change (active incidents, maintenance windows, recent deployments, noisy alerts, escalation contacts).

The policy should define healthy boundaries before the first serious incident tests them.

#### On-Call Policy Blueprint

Every team needs a written on-call policy - not a wiki page someone filled in once, but a living document reviewed quarterly. Use [`templates/on-call-policy-template.md`](../../templates/on-call-policy-template.md) as the maintained, copy-ready starting point; it already covers scope, rotation, compensation, escalation tiers, alert-quality thresholds, and common failure modes.

> **Keep it short.** The template is deliberately ~40 lines. An on-call policy nobody reads because it's 12 pages of legal language is an on-call policy that doesn't exist. Add organizational specifics (pay bands, exact escalation contacts) without bloating the document.
...
> **How this relates to the 180-day rollout.** [Chapter 5's Track B rollout](chapter-05-implementation.md#two-tracks-pilot-vs-full-rollout) is a **process** timeline: the practices, cycles, and artifacts it describes can realistically be running within six months (180 days). Culture is slower and harder to schedule. Finishing the process rollout on time does not, by itself, mean the culture has caught up - institutionalization commonly continues well after the rollout project closes, and how long it takes depends on team size, prior culture, and how consistently leadership reinforces the new behavior. The phases below describe a pattern many teams report, not a guaranteed outcome or a commitment to a date.

**Months 1-3: Foundation Building** (within the process rollout)
...
**Months 4-6: Momentum Building** (the process rollout typically concludes around here)

- Increased engagement and participation, where quick wins have landed
- Visible improvements in team effectiveness, if measured consistently
- Growing stakeholder confidence, provided reporting stays honest
- Framework practices starting to become routine

**Months 7-12: Culture Stabilization** (typically after the process rollout has ended)

- New practices can become "the way we work" - but only where leadership keeps reinforcing them; nothing here is automatic
- Team members may become framework advocates
- Continuous improvement can become natural, given sustained investment
- Cultural change may extend beyond the immediate team

**Year 2+: Culture Evolution** (open-ended; not part of the six-month rollout)

- Framework principles may go on to influence broader organizational decisions
- The team can become a model for other operational groups
- Adaptation and refinement of practices continues indefinitely, if the team keeps investing in it
- Cultural change becomes self-sustaining only where it continues to be rewarded - never assume it, keep checking

> **Reality check.** Culture change doesn't happen because someone printed the values on a mug or added a slide to the onboarding deck. It happens when the first person who says "no, we're not shipping that on a Friday afternoon" gets backed up instead of overruled - and when the engineer who spent a sprint on prevention work nobody noticed still gets a good review. People watch what gets rewarded, not what gets laminated. Treat every month marker above as illustrative, not as a promise this book is making on your team's behalf.
...
A set of concrete actions ops managers can take that require no budget approval and no reorganization - just intention.

| When This Happens                                | Do This Instead of                                                                                                                                              |
| ------------------------------------------------ | --------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| A SEV1 incident is declared                      | Stay out of the technical response. Your job is to handle stakeholder communication, clear blockers, and arrange food/caffeine for the responders.              |
...
| A stakeholder demands a new service be supported | Ask three questions: what's the business justification? what's the SLO? what gets deprioritized to make room?                                                   |
...
> **What not to rate.** Hours worked, tickets closed per week, uptime of systems they don't control, or individual response time (see [Chapter 7 — What Not to Measure](chapter-07-metrics.md#what-not-to-measure)). Rate contribution to team resilience, not individual heroics.
...
1. What we guarantee:
   ...
   - SEV1 incidents will get a human response within 5 minutes, 24/7.
   - We will communicate status within 15 minutes of confirming a SEV1 incident.
...
**Why it matters for SysOps**: This case demonstrates the blameless post-incident review ([Chapter 6, Practice 2](chapter-06-practices.md#2--incident-and-problem-management)) and backup-and-recovery testing ([Chapter 6, Practice 12](chapter-06-practices.md#12--backup--recovery-operations)) advocated throughout this framework. The lesson is cultural before it is technical: psychological safety and transparency turn even a severe failure into durable organizational learning. ([Google's SRE practice makes the same argument for blameless postmortems](https://sre.google/sre-book/postmortem-culture/).)

### Case Study: Netflix - "Reliability Enables Speed" and Chaos Engineering

As Netflix re-architected onto cloud infrastructure at global scale, it adopted a counter-intuitive cultural stance: rather than avoiding failure, teams should **deliberately and continuously inject failure** into production to build confidence that the system withstands real-world turbulence. This discipline, formalized as Chaos Engineering and popularized by tools like Chaos Monkey and the Simian Army, defines experiments that introduce real-world failure variables while minimizing "blast radius" to customers ([Principles of Chaos Engineering](https://principlesofchaos.org/)).
...
- **Automation as a cultural default** - experiments run continuously and automatically, reflecting an organization that designs operations in rather than bolting them on.
